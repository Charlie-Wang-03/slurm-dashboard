...
| Local access | **Any local user on the host can reach the port.** There is no login, so anyone who can connect can view cluster status and submit / cancel jobs as the user who runs the dashboard. Run it only on a host where you trust every local user (your own laptop, a single-user server, or a login node behind a firewall / authenticated reverse proxy). |
