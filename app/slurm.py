...
from app.config import CURRENT_USER, get_settings
...
def get_active_jobs_status() -> Dict[str, str]:
    """One squeue call returning {job_id: label} for the current user's active jobs."""
    try:
        result = subprocess.run(
            ["squeue", "-u", CURRENT_USER, "-h", "-o", "%i|%T"],
            capture_output=True,
            text=True,
            timeout=SLURM_QUERY_TIMEOUT,
        )
