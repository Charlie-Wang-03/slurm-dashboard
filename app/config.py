...
def _build_settings(config: Dict[str, Any]) -> Settings:
    ...
    is_loopback = server_bind_host.lower() == "localhost"
    if not is_loopback:
        try:
            is_loopback = ipaddress.ip_address(server_bind_host).is_loopback
        except ValueError:
            is_loopback = False
    if not is_loopback:
        raise ConfigError(
            "server_bind_host must be a loopback address (127.0.0.0/8, ::1, or localhost); "
            "access remotely via SSH port forwarding"
        )
