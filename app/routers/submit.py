...
MAX_SCRIPT_BYTES = 1024 * 1024  # 1 MiB is ample for a teaching sbatch script.


def _validate_script_size(content: str) -> None:
    if len(content.encode("utf-8")) > MAX_SCRIPT_BYTES:
        raise SlurmSubmitError("Script is too large (maximum 1 MiB)")
...
def list_existing_scripts() -> list:
    ...
    if not scripts_dir.is_dir():
        return []
    try:
        # Validate the directory against the workspace itself. This also
        # rejects a workspace/scripts symlink that escapes the workspace.
        scripts_dir = ensure_path_under_root(scripts_dir, workspace, "script path invalid")
    except ValueError:
        return []
    return sorted(
...
async def submit_job(
    ...
    try:
        ...
        if existing_script:
            ...
            raw_content = await script_file.read(MAX_SCRIPT_BYTES + 1)
            if len(raw_content) > MAX_SCRIPT_BYTES:
                raise SlurmSubmitError("Script is too large (maximum 1 MiB)")
            content = raw_content.decode("utf-8", errors="replace")
            source = "upload"
            filename = script_file.filename
        _validate_script_size(content)
    ...
    except (SlurmSubmitError, ValueError, OSError) as exc:
